...
fn render_form(
    ...
    let example_rows = examples(draft.kind)
        .len()
        .min(content_height.saturating_sub(2));
    let fixed_height = 2 + example_rows;
    ...
    let url_y = message_y
        .saturating_sub(fixed_height as u16)
        .saturating_add(1);
    ...
    for (index, example) in examples(draft.kind).iter().take(example_rows).enumerate() {
        frame.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled(
                    format!("  {:<20}", if index == 0 { "EXAMPLES" } else { "" }),
            ...
            Rect::new(
                inner.x,
                url_y.saturating_add(1 + index as u16),
                inner.width,
                1,
            ),
        );
