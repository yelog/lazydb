...
use crate::{
    ...
    model::{
        ...
        data_query::{
            ...
            DataQueryOptions, DataQueryState,
        },
        editor::{EditorMode, EditorRenderSnapshot, EditorViewport},
        execution_target::{ExecutionTarget, resolve_default_target},
        explorer::{
        ...
        pagination::{PageRequest, PageSize, ResultPagination},
...
impl App {
    ...
    fn dashboard_metadata_commands(&mut self, connection: ConnectionIdentity) -> Vec<Command> {
        let tabs = self
            .tabs
            .iter()
        ...
        let mut records = self
            ...
            .filter(|record| {
                ...
                [
                    ...
                    target.map(|target| target.database.as_str()),
                    target.and_then(|target| target.schema.as_deref()),
                ]
                .into_iter()
                .flatten()
    ...
    fn append_workspace(&mut self, workspace: ConnectionWorkspace) {
        ...
        for tab in &workspace.tabs {
            if let WorkspaceTab::Relation(relation) = tab {
                if self.editor.has_session(relation.ddl_editor_id) {
                    continue;
                }
                let text = match &relation.ddl {
                ...
                self.editor.open_read_only(relation.ddl_editor_id, &text);
            }
        }
        self.tabs.extend(
            workspace
                .tabs
                .into_iter()
                .filter(|tab| !existing_ids.contains(&tab.id())),
        );
        let existing_console_ids = self
            .sql_editors
            .iter()
            .map(|record| record.id)
            .collect::<HashSet<_>>();
        self.sql_editors.extend(
            workspace
                .sql_editors
                .into_iter()
                .filter(|record| !existing_console_ids.contains(&record.id)),
        );
    }
    ...
    fn activate_profile_workspace(
        ...
        if self.active_workspace_profile != Some(profile_id) || self.tabs.is_empty() {
            if self.active_workspace_profile != Some(profile_id)
                && let Some((old_profile_id, workspace)) = self.snapshot_active_workspace()
            {
            ...
            if let Some(active_tab) = self
                .tabs
                .iter()
                .position(|tab| workspace_tab_ids.contains(&tab.id()))
            {
    ...
    pub fn workspace_snapshot(&self) -> WorkspaceSnapshot {
        ...
        let (global_consoles, global_tabs) = if active_profile.is_none()
            && !has_profile_workspaces
            && !has_unpersisted_default_console
        {
            let workspace = self.persisted_workspace_from_parts(
                Uuid::nil(),
                &self.tabs,
                &self.sql_editors,
                self.active_tab_id(),
            );
            (workspace.consoles, workspace.tabs)
        } else {
    ...
    pub fn restore_workspace(
        ...
        self.recent_targets = snapshot.recent_targets.clone();
        ...
        for persisted in consoles {
            let open = persisted.open || global_tabs.iter().any(|tab| {
                    matches!(tab, PersistedTab::Console { console_id } if *console_id == persisted.id)
                });
    ...
    pub fn update(&mut self, action: Action) -> Vec<Command> {
        ...
        match action {
            ...
            Action::OpenConsoleTargetSelector { console_id } => {
                ...
                let candidates = self.execution_target_candidates_all_profiles();
                let selected = candidates
                    .iter()
                    .position(|candidate| console.execution_target.as_ref() == Some(candidate))
                    .unwrap_or(0);
                self.overlay = Some(Overlay::TargetSelector {
                    candidates,
                    selected,
            ...
            Action::ConfirmTargetSelector => {
                ...
                if let Some(console_id) = console_id {
                    return self.bind_console_target(console_id, target);
                }
                let Some(tab) = self.active_console_opt() else {
                    return Vec::new();
                };
            ...
            Action::RelationSucceeded { request, snapshot } => {
                let request_tab_id = request.tab_id;
                let commands = self.accept_relation(request, Ok(*snapshot));
                if self
                    .active_tab_id()
                    .is_some_and(|tab_id| tab_id == request_tab_id)
                {
                    self.refresh_active_data_query_completion();
                }
                commands
            ...
            Action::ConnectionSucceeded {
                ...
                for console_id in ready_pending {
                    ...
                    let draft = sql::ExecutionDraft::new(
                        ...
                        target.clone(),
                        pending.transaction_generation,
                        pending.document_revision,
                        pending.scope,
                        pending.source,
                        pending.sql,
                        pending.dialect,
                        pending.transaction_mode,
                        pending.transaction_state,
                    );
                    commands.extend(self.dispatch_draft(draft));
                }
                commands
            }
            ...
            Action::ConnectionInvalidated {
                ...
                for tab in &mut self.tabs {
                    ...
                    let connection_matches = tab.execution_connection == Some(connection)
                        || (tab.execution_connection.is_none()
                            && tab
                                .execution_target
                                .as_ref()
                                .is_some_and(|target| target.profile_id == connection.profile_id));
                    ...
                    if connection_matches && tab.transaction_state != TransactionState::Idle {
            ...
            Action::CatalogSearchFailed {
                ...
                if let Some(search) = self.explorer.search.as_mut().filter(|search| {
            ...
            Action::QueryFinished {
                ...
                let valid = self.console_query_matches(tab_id, generation, connection);
                if !valid {
                    return Vec::new();
                }
                let impact = self
            ...
            Action::QueryFailed {
                ...
                if !self.console_query_matches(tab_id, generation, connection) {
                    return Vec::new();
                }
                let Some(tab) = self
                    .tabs
            ...
            Action::RelationEditBooleanMove(direction) => {
                self.relation_edit_boolean(|input| input.move_boolean(direction));
                Vec::new()
            }
            Action::RelationEditBooleanToggle => {
                self.relation_edit_boolean(|input| {
                    if let Some(value) = input.boolean_selection() {
                        input.set_boolean(!value);
                    }
                });
                Vec::new()
            }
            Action::RelationEditBooleanSet(value) => {
            ...
            Action::RelationEditTemporalMonth(direction) => {
            ...
            Action::RelationEditJsonFormat => {
                self.relation_edit_json_format();
                Vec::new()
            }
            Action::RelationEditJsonMove(direction) => {
                self.relation_edit_json_move(direction);
                Vec::new()
            }
            Action::RelationEditBackspace => {
                self.relation_edit_apply_text_edit(
            ...
            Action::RelationEditDelete => {
                self.relation_edit_apply_text_edit(crate::model::text_input::TextInputEdit::Delete);
                Vec::new()
            }
            Action::RelationEditMoveLeft => {
                self.relation_edit_apply_text_edit(
            ...
            Action::RelationEditUndo => {
                self.relation_edit_apply_text_edit(crate::model::text_input::TextInputEdit::Undo);
                Vec::new()
            }
            Action::RelationEditRedo => {
                self.relation_edit_apply_text_edit(crate::model::text_input::TextInputEdit::Redo);
                Vec::new()
            }
            ...
            Action::RelationMutationFailed {
                ...
                message,
                diagnostic,
            } => self.relation_mutation_result(request, Err((message, diagnostic))),
            Action::RelationCommitted {
                tab_id,
                generation,
                connection,
            ...
            .filter(|tab| {
                tab.as_console().is_some_and(|console| {
                    console
                        ...
                        .as_ref()
                        .is_some_and(|target| target.profile_id == profile_id)
                        && self.transaction_needs_exit(console.id)
                })
            })
            .map(WorkspaceTab::id)
            .collect::<Vec<_>>();
    ...
    fn remember_target(&mut self, target: &ExecutionTarget) {
        ...
        self.recent_targets.insert(0, target.clone());
        self.recent_targets.truncate(16);
    }
    ...
    fn profile_saved(
        ...
        let mut commands = if scope_changed {
            self.cancel_relation_requests_for_connection(None)
        } else {
            Vec::new()
        };
        add_explorer_profile(
    ...
    fn request_connection(&mut self, profile_id: Uuid) -> Vec<Command> {
        ...
        if self.connection.profile_id != Some(profile_id) {
            match self.profile_exit_check(self.connection.profile_id.unwrap_or(profile_id)) {
                WorkspaceExitCheck::Ready | WorkspaceExitCheck::ConsoleTransactions(_) => {
                    if matches!(
                        self.workspace_exit_check(),
                        WorkspaceExitCheck::RelationTransaction
                    ) {
                ...
                WorkspaceExitCheck::Running => {
                    self.notify_warning(
                        "Connection",
                        "Wait for running SQL or relation loads to finish before switching connections",
                    );
                    return Vec::new();
                }
                WorkspaceExitCheck::RelationTransaction => {
    ...
    fn bind_console_target(&mut self, console_id: Uuid, target: ExecutionTarget) -> Vec<Command> {
        ...
        if tab.transaction_state != TransactionState::Idle {
            self.notify_warning(
                "LazyDB",
                "Commit or roll back the active transaction before rebinding this console",
            );
            return Vec::new();
        }
        if tab.execution_target.as_ref() == Some(&target) {
        ...
        tab.output.clear();
        tab.result_view = ResultView::Data;
        tab.grid = DataGridState::default();
        tab.pagination = ResultPagination::from_page(PageRequest::first(PageSize::default()), 0);
        tab.completion = None;
        tab.completion_request = None;
        tab.last_execution = None;
        tab.target_error = None;
        tab.semantic_diagnostics.clear();
        tab.derived = None;
        tab.query = DataQueryState::default();
        self.editor.open_read_only(tab.output_editor_id, "");
        if let Some(record) = self
            .sql_editors
            .iter_mut()
                    ...
                    && (tab.execution_connection == Some(connection)
                        || (tab.execution_connection.is_none()
                            && tab
                                .execution_target
                                .as_ref()
                                .is_none_or(|target| target.profile_id == connection.profile_id))
                        || tab.last_execution.as_ref().is_some_and(|last| {
                            last.draft.connection == connection
                                && last.draft.query_generation + 1 == generation
                        }))
            })
    }
    ...
    fn editor_diagnostics_key(&self) -> Option<crate::sql::DiagnosticScheduleKey> {
        ...
        Some(crate::sql::DiagnosticScheduleKey {
            console_id: tab.id,
            document_revision: self.active_editor_revision(),
            connection: tab.execution_connection,
    ...
    fn run_active_sql(&mut self, full_buffer: bool) -> Vec<Command> {
        ...
        let connection = self.database_command_identity();
        let console_connection = self.active_console_opt().and_then(|tab| {
    ...
    fn dispatch_draft(&mut self, draft: sql::ExecutionDraft) -> Vec<Command> {
        ...
        tab.execution_connection = Some(draft.connection);
    ...
    fn relation_edit_cell(&mut self) {
        ...
        let description = database_kind.and_then(|kind| {
            result.as_ref().and_then(|result| {
                result.columns.get(column).and_then(|column| {
                    crate::model::cell_editor::classify_column_type(kind, &column.type_name)
                })
            })
        });
    ...
    fn relation_edit_apply_text_edit(&mut self, edit: crate::model::text_input::TextInputEdit) {
    ...
    fn relation_edit_json_move(&mut self, direction: isize) {
        if let Some(edit_session) = self.relation_session_mut()
            && let RelationGridMode::EditCell(state) = &mut edit_session.mode
    ...
    fn relation_edit_json_format(&mut self) {
        if let Some(edit) = self.relation_session_mut()
            && let RelationGridMode::EditCell(state) = &mut edit.mode
            && let Some(json) = state.input.json_buffer_mut()
        {
            if let Ok(formatted) = json.format() {
                ...
                state.error = json
                    .validate()
                    .err()
                    .map(|error| format!("{} at {}:{}", error.message, error.line, error.column));
            }
        }
    }

    fn relation_edit_boolean(
        &mut self,
        operation: impl FnOnce(&mut crate::model::cell_editor::CellEditorBuffer),
    ) {
        if let Some(edit) = self.relation_session_mut()
            && let RelationGridMode::EditCell(state) = &mut edit.mode
        {
    ...
    fn relation_edit_confirm(&mut self) -> Vec<Command> {
        ...
        let value = match value {
            ...
            Err(message) => {
                if let Some(WorkspaceTab::Relation(tab)) = self.tabs.get_mut(self.active_tab) {
                    if let Some(RelationGridMode::EditCell(state)) =
                        tab.edit.as_mut().map(|edit| &mut edit.mode)
                    {
        ...
        if let Some(edit) = self.relation_session_mut() {
            if edit.update_cell(state.row, state.column, value) {
                edit.mode = RelationGridMode::Browse;
            }
        }
        Vec::new()
    ...
    fn relation_save(&mut self) -> Vec<Command> {
        ...
        if has_unversioned_delete {
            self.notify_warning(
                "Relation",
                "Cannot delete PostgreSQL rows without a captured row version; refresh the relation first",
            );
            return Vec::new();
        }
        ...
        let Some((mut requests, snapshot)) =
            self.relation_context(|tab, edit, columns, metadata, target, scope| {
                ...
                if let Some(first) = deleted.first() {
                    requests.push(request(first.row_id, RelationMutation::DeleteRows(deleted)));
                }
    ...
    fn relation_mutation_result(
        ...
        result: Result<
            crate::db::mutation::MutationResult,
            (
                String,
                Option<crate::db::transaction::RelationMutationDiagnostic>,
            ),
        >,
    ) -> Vec<Command> {
        ...
        match result {
            Ok(crate::db::mutation::MutationResult::Updated { row, version }) => {
            ...
            Ok(crate::db::mutation::MutationResult::Deleted { .. }) => {
                ...
                if let crate::db::mutation::RelationMutation::DeleteRows(rows) = &request.operation
                {
                    for mutation in rows {
                        if let Some(row) =
                            edit.rows.iter_mut().find(|row| row.id == mutation.row_id)
                        {
            ...
            Ok(crate::db::mutation::MutationResult::Inserted { row, version }) => {
                if let Some(r) = edit.rows.iter_mut().find(|r| r.id == request.row_id) {
                    r.mark_inserted(row, version);
                }
                if edit.pending_mutation_history.is_none() {
                    edit.record_mutation(RelationMutationHistory {
                        forward: request.clone(),
            ...
            Err((message, diagnostic)) => {
                ...
                for id in ids {
                    if let Some(r) = edit.rows.iter_mut().find(|r| r.id == id) {
                        let is_conflict = diagnostic.as_ref().is_some_and(|detail| {
                            matches!(
                                detail.category,
                                crate::db::transaction::RelationMutationCategory::Conflict
                            )
                        }) || (diagnostic.is_none()
        ...
        match (request.kind, result) {
            (RelationRequestKind::Preview, Ok(RelationSnapshot::Preview(snapshot))) => {
                if matches!(&tab.data, RelationLoad::Loading { request: pending, .. } if pending == &request)
                {
                    ...
                    let row_versions = snapshot.row_versions.clone();
                    let rows = snapshot
                        .result
                        .result_sets
                        .last()
                    ...
                    tab.grid.selected_row = 0;
                    tab.grid.row_offset = 0;
                    tab.edit = rows.map(|rows| {
                        RelationEditSession::from_rows_with_versions(rows, row_versions)
                            .expect("relation preview row versions must match rows")
                    });
                }
            }
...
fn parse_relation_value(
    ...
    if !matches!(old, CellValue::Text(_)) && value.eq_ignore_ascii_case("null") {
...
mod tests {
    ...
    use crate::{
        ...
        model::relation_edit::RelationMutationHistory,
        model::tab::{
    ...
    #[test]
    fn dashboard_metadata_response_is_owned_by_profile_not_active_connection() {
        let first = import_connection_url("postgres://localhost/first", Some("first"))
            .unwrap()
            .profile;
        let second = import_connection_url("postgres://localhost/second", Some("second"))
            .unwrap()
            .profile;
        ...
        dashboard.profile_id = Some(first_id);
        dashboard.connection = Some(ConnectionIdentity {
            profile_id: first_id,
            generation: 1,
        });
        let tab_id = dashboard.id;
        app.tabs.push(WorkspaceTab::Dashboard(dashboard));
        let mut other = crate::model::dashboard::DashboardTab::new();
        other.profile_id = Some(second_id);
        app.tabs.push(WorkspaceTab::Dashboard(other));

        let commands = app.dashboard_metadata_commands(ConnectionIdentity {
            profile_id: first_id,
            generation: 1,
        });
        ...
        assert!(matches!(
            commands[0],
            Command::LoadDashboardMetadata { tab_id: command_tab, .. } if command_tab == tab_id
        ));

        app.update(Action::DashboardMetadataLoaded {
            tab_id,
            tab_generation: 0,
            connection: ConnectionIdentity {
                profile_id: first_id,
        ...
        assert!(tab.metadata_error.is_none());
    }

    #[test]
    fn dashboard_response_from_another_profile_is_ignored_without_clearing_loading() {
        let first = import_connection_url("postgres://localhost/first", Some("first"))
            .unwrap()
            .profile;
        let second = import_connection_url("postgres://localhost/second", Some("second"))
    ...
    fn relation_mutation_failure_marks_conflict_and_requests_rollback() {
        ...
        let commands = app.update(Action::RelationMutationFailed {
            ...
            diagnostic: Some(crate::db::transaction::RelationMutationDiagnostic {
                category: crate::db::transaction::RelationMutationCategory::Conflict,
                sqlstate: None,
    ...
    #[test]
    fn relation_mutation_type_failure_does_not_mark_row_as_conflict() {
        let (mut app, request) = relation_mutation_app();
        let commands = app.update(Action::RelationMutationFailed {
            request,
            message: "update: value cannot be assigned (SQLSTATE 22P02)".into(),
            diagnostic: Some(crate::db::transaction::RelationMutationDiagnostic {
                category: crate::db::transaction::RelationMutationCategory::TypeMismatch,
                sqlstate: Some("22P02".into()),
                operation: "update".into(),
                relation: "items".into(),
                context: Some("safe type context".into()),
                message: "value cannot be assigned".into(),
            }),
        });
        let WorkspaceTab::Relation(tab) = &app.tabs[app.active_tab] else {
            panic!("expected relation tab")
    ...
    #[test]
    fn failed_relation_mutation_clears_remaining_writes_and_restores_full_snapshot() {
        let mut app = App::new(Vec::new());
        let mut snapshot = RelationEditSession::from_rows_with_versions(
            vec![
                vec![CellValue::Integer(1), CellValue::Text("old".into())],
                vec![CellValue::Integer(2), CellValue::Text("keep".into())],
            ],
            Some(vec![
                crate::db::mutation::RowVersion::PostgresXmin(11),
                crate::db::mutation::RowVersion::PostgresXmin(22),
            ]),
        )
        .unwrap();
        let snapshot_ids = snapshot.rows.iter().map(|row| row.id).collect::<Vec<_>>();
        snapshot.mutation_undo.push(RelationMutationHistory {
            forward: relation_mutation_app().1.clone(),
            inverse: relation_mutation_app().1,
        });
        let mut draft = snapshot.clone();
        draft.update_cell(0, 1, CellValue::Text("changed".into()));
        draft.delete_rows(1..=1);
        draft
            .pending_save
            .push_back(draft.mutation_undo[0].forward.clone());
        ...
        let WorkspaceTab::Relation(tab) = app.tabs.iter().find(|tab| tab.id() == tab_id).unwrap()
        else {
            panic!()
        };
        let edit = tab.edit.as_ref().unwrap();
        assert_eq!(
            edit.rows.iter().map(|row| row.id).collect::<Vec<_>>(),
            snapshot_ids
        );
        assert_eq!(edit.rows, snapshot.rows);
        assert!(edit.pending_save.is_empty());
        assert_eq!(edit.mutation_undo, snapshot.mutation_undo);
    }

    #[test]
    fn failed_or_unknown_relation_rollback_keeps_snapshot_and_never_reports_success() {
        for unknown in [false, true] {
            let mut app = App::new(Vec::new());
            let snapshot = RelationEditSession::from_rows_with_versions(
                vec![vec![CellValue::Integer(1)]],
                Some(vec![crate::db::mutation::RowVersion::PostgresXmin(7)]),
            )
            .unwrap();
            let mut tab = RelationTab::new("items");
            tab.transaction_state = TransactionState::RollingBack;
            ...
            app.relation_transaction_finished(
                ...
                false,
                Some(("rollback failed".into(), unknown)),
            );

            let WorkspaceTab::Relation(tab) =
                app.tabs.iter().find(|tab| tab.id() == tab_id).unwrap()
            else {
                panic!()
            };
            assert_eq!(tab.transaction_snapshot, Some(snapshot));
            assert_eq!(
                tab.transaction_state,
                if unknown {
                    TransactionState::OutcomeUnknown
            ...
            assert!(app.notifications.history().next().is_some());
        }
    }
    ...
    #[test]
    fn visible_console_records_use_name_order_without_default_or_open_priorities() {
        let mut app = App::new(Vec::new());
        app.sql_editors = vec![
    ...
    #[test]
    fn console_manager_order_is_name_based_not_open_state_based() {
        let mut app = App::new(Vec::new());
        app.sql_editors = vec![
    ...
    #[test]
    fn closing_the_last_console_leaves_an_empty_workspace() {
        let mut app = App::new(Vec::new());
